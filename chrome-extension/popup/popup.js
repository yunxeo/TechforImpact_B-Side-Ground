...
function getPuriComment(stat) {
  ...
  if (highRatio > 0.5) {
    return {
      ...
      msg: isPast
        ? pick([
            `${n}번 전송했어요. 긴 입력이 많았지만 그만큼 열심히 사용한 하루였어요.`,
            `총 ${n}번 보냈어요. 내용이 많았던 만큼 AI를 적극 활용한 날이었네요.`,
            `${n}번 채팅했어요. 긴 작업들로 가득 찬 하루였군요!`
          ])
        : pick([
            `${n}번 전송했어요. 오늘 AI를 열심히 활용했네요! 긴 것들은 나눠보면 더 좋은 답이 올 수 있어요.`,
            `총 ${n}번 보냈어요. 긴 내용도 많았는데 수고했어요! 주제를 나눠서 물어보면 답이 더 또렷해질 거예요.`,
            `${n}번 채팅했어요. 오늘 정말 많이 활용했군요! 다음엔 하나씩 나눠서 물어봐도 좋아요.`
          ])
    };
  }

  if (highRatio > 0.2) {
    const high = stat.highCount;
    return {
      imgKey: "medium",
      msg: isPast
        ? pick([
            `${n}번 전송했어요. ${high}번은 길었지만 나머지는 간결했네요. 잘했어요!`,
            `총 ${n}번 보냈어요. 대부분 짧게 잘 쓰셨어요!`,
            `${n}번 채팅했어요. 전반적으로 균형 잡힌 하루였어요.`
          ])
        : pick([
            `${n}번 전송했어요. 대부분 간결하게 잘 쓰셨어요! 👍`,
            `총 ${n}번 보냈어요. ${high}번 정도 길었지만 나머지는 훌륭했어요!`,
            `${n}번 채팅했어요. 전반적으로 좋은 습관을 유지하고 있어요 🌿`
          ])
    };
  }
  ...
  return {
    ...
    msg: isPast
      ? pick([
          `${n}번 전송했고, 대부분 짧고 간결했어요. 훌륭한 하루였네요!`,
          `총 ${n}번 보냈는데 입력이 전반적으로 딱 좋았어요.`,
          `${n}번 채팅했어요. 간결하게 잘 쓴 하루였어요 🌱`
        ])
      : pick([
