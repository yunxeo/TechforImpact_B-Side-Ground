...
function getPuriComment(stat) {
  ...
  const bdStr = breakdown ? ` (${breakdown})` : "";

  if (highRatio > 0.5) {
    const nudge = allHigh
      ? " 전부 길었으니 주제를 나눠서 물어보면 답이 훨씬 또렷해질 거예요!"
      : " 긴 것들은 나눠서 물어보면 더 정확한 답을 받을 수 있어요.";
    return {
      imgKey: "high",
      msg: isPast
        ? pick([
            `총 ${n}번 전송했어요${bdStr}. 긴 작업들로 가득 찬 하루였군요!`,
            `${n}번 보냈어요${bdStr}. AI를 적극 활용한 날이었네요.`,
            `${n}번 채팅했어요${bdStr}. 내용이 많았던 만큼 열심히 한 하루였어요.`
          ])
        : pick([
            `총 ${n}번 전송했어요${bdStr}.${nudge}`,
            `${n}번 보냈어요${bdStr}. 오늘 AI를 열심히 활용했네요!${nudge}`,
            `${n}번 채팅했어요${bdStr}. 수고했어요!${nudge}`
          ])
    };
  }

  if (highRatio > 0.2) {
    const nudge = noLow ? " 간결한 입력을 조금 더 늘려봐요!" : "";
    return {
      imgKey: "medium",
      msg: isPast
        ? pick([
            `총 ${n}번 전송했어요${bdStr}. 대부분 잘 쓰셨어요!`,
            `${n}번 보냈어요${bdStr}. 전반적으로 균형 잡힌 하루였어요.`,
            `${n}번 채팅했어요${bdStr}. 긴 것도 있었지만 잘 활용했어요!`
          ])
        : pick([
            `총 ${n}번 전송했어요${bdStr}. 대부분 간결하게 잘 쓰셨어요! 👍${nudge}`,
            `${n}번 보냈어요${bdStr}. 전반적으로 좋은 습관을 유지하고 있어요 🌿${nudge}`,
            `${n}번 채팅했어요${bdStr}. 훌륭해요!${nudge}`
          ])
    };
  }
  ...
  return {
    ...
    msg: isPast
      ? pick([
          `총 ${n}번 전송했어요${bdStr}. 간결하게 잘 쓴 하루였어요!`,
          `${n}번 보냈어요${bdStr}. 입력이 전반적으로 딱 좋았어요 🌱`,
          `${n}번 채팅했어요${bdStr}. 훌륭한 하루였네요!`
        ])
      : pick([
          `총 ${n}번 전송했어요${bdStr}. 오늘도 훌륭한 습관이에요! 🌱`,
          `${n}번 보냈어요${bdStr}. 짧고 명확한 질문들이 많았어요. 이 페이스 유지해요!`,
          `${n}번 채팅했어요${bdStr}. 입력 길이가 딱 좋았어요. 덕분에 답변도 빨랐을 거예요 ✨`
        ])
  };
}
