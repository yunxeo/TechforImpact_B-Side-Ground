...
function getPuriComment(stat) {
  ...
  if (highRatio > 0.2) {
    ...
    return {
      ...
      msg: isPast
        ? pick([
            `${n}번 전송했고, 그 중 ${high}번은 긴 입력이었어요.`,
            `총 ${n}번 보냈어요. ${high}번은 길었지만 나머지는 간결했네요.`,
            `${n}번 채팅 중 ${high}번이 길었어요. 전반적으로 무난한 하루였어요.`
          ])
        : pick([
            `${n}번 보냈고, ${high}번은 조금 길었어요. 핵심만 남기면 더 빨라져요!`,
            `총 ${n}번 전송했어요. ${high}번 정도 길었으니 다음엔 조금만 더 줄여봐요 🌿`,
            `${n}번 채팅했어요. ${high}번이 길었지만 많이 줄었어요. 이 방향 유지해봐요!`
          ])
    };
  }
